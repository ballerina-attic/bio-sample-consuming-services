...
def main():
    with open('src/scripts/consuming_services.bal', 'r') as action_obj:
        updated_bal_file = action_obj.read()
        with open('consuming-services.md', 'a') as write_obj:
            write_obj.write("```\n")
    ...
    with open('consuming-services.md','r') as commit_obj:
        updated_file = commit_obj.read()
        
    commit_message = 'Update Ballerina in Action samples'
    try:
        update = commit_file('components/home-page/bal-action/action-bbe/consuming-services.md', updated_file, 'update-consuming-services', commit_message)[0]

        if update:
            pr_title = '[Automated] Update Ballerina in Action samples'
            pr_body = 'Update Ballerina in Action samples for source code changes in `Consuming Services`'
            head_branch = 'update-consuming-services'
